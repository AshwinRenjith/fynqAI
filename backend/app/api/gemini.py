...
async def chat_with_gemini(request: ChatRequest, current_user_id: str = Depends(validate_token), db: Session = Depends(get_db), rate_limit: RateLimiter = Depends(RateLimiter(times=5, seconds=60))):
...
async def chat_with_gemini_image(image: UploadFile = File(...), message: str = Form(...), chat_id: int | None = Form(None), current_user_id: str = Depends(validate_token), db: Session = Depends(get_db), rate_limit: RateLimiter = Depends(RateLimiter(times=2, seconds=300))):
...
async def get_chat_history(current_user_id: str = Depends(validate_token), db: Session = Depends(get_db), rate_limit: RateLimiter = Depends(RateLimiter(times=10, seconds=60))):
